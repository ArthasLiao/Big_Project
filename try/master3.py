...
def mouse_callback(event, x, y, zuobiao1, outputs1):
    global center, tracking, cancel_tracking

    if event == cv2.EVENT_LBUTTONDOWN:
        # 点击人体框，开始跟踪
        for i in range(len(outputs1)):  # range(len(outputs)),deepsort_tracker[:, :4]
            if zuobiao1[i][0] < x < zuobiao1[i][0] and zuobiao1[i][0] < y < zuobiao1[i][0]:
                #data.tracking = True
                cancel_tracking = False
                break
    ...
    while mycap.ifcontinue():
        ...
        if ret:
            # 开始检测图片中的人
            np_img,zuobiao2,outputs2=Counting_Processing(img, yolo5_config, Model, class_names, deepsort_tracker, Obj_Counter)
            result_img = np_img
            # print(result_img)
            ...
            cv2.setMouseCallback("video", mouse_callback(event,x,y,zuobiao2,outputs2))
            cv2.waitKey(t)
